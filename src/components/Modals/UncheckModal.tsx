...
export default function UncheckModal({ target, open, onClose }: ModalProps) {
						...
						>
							<Dialog.Panel className="border border-zinc-800 relative transform overflow-hidden rounded-lg bg-zinc-950 px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
								<div className="sm:flex sm:items-start">
									<div className="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-yellow-900 sm:mx-0 sm:h-10 sm:w-10">
									...
									<div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
										<Dialog.Title as="h3" className="text-base font-semibold leading-6 text-gray-200">
											Unmark answer #{target.id} as accepted?
										</Dialog.Title>
										<div className="mt-2">
											<p className="text-sm text-gray-300">
												The answer will be unmarked as accepted.
											</p>
										</div>
									</div>
								</div>
								<div className="mt-5 sm:ml-10 sm:mt-4 sm:flex sm:pl-4">
									<button
										type="button"
										className="inline-flex w-full justify-center rounded-md bg-yellow-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-yellow-700 sm:w-auto"
										disabled={!enabled}
										onClick={() => close(true)}
									>
										Uncheck

...
export default function QuestionCard({ data }: QuestionProps) {
	return (
		<li
			key={data.id}
			className="relative py-5 pl-4 pr-6 hover:bg-zinc-800 sm:py-6 sm:pl-6 lg:pl-8 xl:pl-6"
		>
			<div className="flex items-center justify-between space-x-4">
				...
				<div className="sm:hidden">
					<ChevronRightIcon className="h-5 w-5 text-gray-200" aria-hidden="true" />
				</div>
				{/* Repo meta info */}
				<div className="hidden flex-shrink-0 flex-col items-end space-y-3 sm:flex">
					<div className="flex items-center space-x-4">
						<p
							className="relative text-sm text-gray-200"
						>
							{data.answers.length} {data.answers.length == 1 ? "answer" : "answers"}
						</p>
					</div>
					<p className="flex space-x-2 text-sm text-gray-500">
						{formatDate(new Date(Date.parse(data.created_at.toString())))}
					</p>
				</div>
			</div>
		</li>
